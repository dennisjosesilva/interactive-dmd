...
The software needs a C++ compiler, the CUDA, conan, graphviz, and Qt to build. The program depends on some custom packages. To install a conan package, we need to run a conan command at the root of the repositories on the links down below. So before running the program, we need to install these packages available in the links down below:
