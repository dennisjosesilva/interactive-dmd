...
Apart from that, users can also check the 'Interp' checkbox to get an interpolated result; check/uncheck the 'Upper' checkbox to run upper/lower thresholding scheme, see details in this [paper](https://www.mdpi.com/2313-433X/7/8/153). After setting all parameters, one can run 'Run' button to get the SDMD reconstructed result. Users can also click the 'OrigImg' checkbox to check the original image.
